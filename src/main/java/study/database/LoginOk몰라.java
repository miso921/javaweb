...
@WebServlet("/database/LoginOkfgf")
public class LoginOk몰라 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(vo.getName() != null) { // null이 아닐때니까 정상이므로 정회원
			...
			dao.setPointPlus(mid); // 중복되지 않는 mid를 넘긴다
			...
			out.print("location.href='"+request.getContextPath()+"/study/0428_database/login.jsp';");  // location.href는 직렬화가 아니고 jsp로 보내야하기 때문에 redirect와 같은 것으로 request.getContextPath() 사용해야함!
			out.print("</script>");
		}
	}
}
