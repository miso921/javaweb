...
<div class='container'>
	...
	<pre>
		...
		사용법 : < fmt : 명령어 value="$ {값/변수}" pattern="표현 패턴" type="화폐 단위" />
	</pre>
	<div>
		1-1. 천단위 쉼표 표시<br /> 
			...
			7654.321 => <fmt:formatNumber value="${won2}" pattern="0,000,000.0000" /><br /><br />
			
		1-2. 화폐단위<br />	
			7654.321 => <fmt:formatNumber value="${won2}" pattern="#,##0.0" /><br /> <!-- 0은 의미있는 숫자, #은 의미없는 숫자 -->
			7654321 (원화) => <fmt:formatNumber value="${won}" type="currency" /><br /> <!-- 윈도우 국가 설정에 따라 기호가 다르게 나온다. -->
		...
		2. 날짜<br />
		 	...
		 	<fmt:formatDate value="${today}" pattern="yyyy년 MM월 dd일 hh시 mm분 ss초" /><br /><br />
		
		3. 국가별 설정(로케일)<br />
			톰캣서버의 기본 로케일 : <%=response.getLocale()%><br />
			톰캣서버의 기본 로케일 변경(미국식) : <fmt:setLocale value="en_US"/> <!-- 윈도우 국가 설정을 '미국'으로 변경만 한 것 -->
																		 <fmt:formatNumber value="${won2}" type="currency" /><br /> 
		<hr />기타<hr />																 
		4. URL이동 : location.href = ''; => redirect<br />
			 <%-- jstl3.jsp로 이동하기 : <c:redirect url="jstl3.jsp" /> --%><br /><br />
		5. 절대경로 : <br />
			<img src="<%=request.getContextPath()%>/images/1.jpg" width="150px" /><br />
			<img src="${ctp}/images/6.jpg" width="150px" /><br /><br />
		6. import : <br /> 
